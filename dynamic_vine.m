function [Correlation,VarCov,parameters_vine,parameters_raw] = dynamic_vine(data,method,level)

% C-vine GARCH estimation, with underlying structure given by the C-vine
% tree structure, and with the innovation process generated under the
...
if N==2
    ...
    parfor jj = ii+1:N
        ...
        [param,~,~,~,~,~]=fmincon(@(x)reduced_likelihood_pcorr(x,ii,[data(:,1:ii),data(:,jj)],[h(:,1:ii),h(:,jj)],kappa),[0.001+(0.05-0.001)*rand(1),0.85+(0.95-0.85)*rand(1),0.01+(0.02-0.01)*rand(1)],[],[],[],[],[],[],@(x)vine_constr(x),opts);
        [~,~,~,partial] = reduced_likelihood_pcorr(param,ii,[data(:,1:ii),data(:,jj)],[h(:,1:ii),h(:,jj)],kappa);
        ...
        case 'no-truncation'
            for ii = 3:N-1
                ...
                parfor jj = ii+1:N
                    ...
                    [param,~,~,~,~,~]=fmincon(@(x)reduced_likelihood_pcorr(x,ii,[data(:,1:ii),data(:,jj)],[h(:,1:ii),h(:,jj)],kappa),[0.001+(0.05-0.001)*rand(1),0.85+(0.95-0.85)*rand(1),0.01+(0.02-0.01)*rand(1)],[],[],[],[],[],[],@(x)vine_constr(x),opts);
                    [~,~,~,ppartial] = reduced_likelihood_pcorr(param,ii,[data(:,1:ii),data(:,jj)],[h(:,1:ii),h(:,jj)],kappa);
